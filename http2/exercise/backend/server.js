...
server.on("stream", (stream, headers) => {
  // pay attention to the correct path and method
  const path = headers[":path"];
  const method = headers[":method"];
  // because streams are open for every request from the browser

  if (path === '/msgs' && method === "GET") {
    // immediately reply with 200 ok
    console.log('connected ', + stream.id);
    stream.respond({
      ":status": 200,
      "content-type": "text/plain; charset=utf-8" // JSON doesn't stream
    })
